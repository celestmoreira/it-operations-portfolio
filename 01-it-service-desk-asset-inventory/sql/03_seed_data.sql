...
-- Each query maps to a real IT operations or onboarding task.
-- Run them one at a time and screenshot the results.
-- =====================================================================
...
-- A1. Full inventory with who has what and where (the master view).
SELECT  a.asset_tag,
        a.asset_type,
        a.manufacturer,
        a.model,
...
-- A2. Asset count by status (how much is deployed vs sitting idle vs broken).
SELECT  status, COUNT(*) AS asset_count
FROM    assets
GROUP BY status
ORDER BY asset_count DESC;

-- A3. Asset count and total value by location (where's the money sitting).
...
--     This is the "warranty status" the resume bullet calls out.
SELECT  a.asset_tag,
        a.manufacturer,
        a.model,
...
--     Use this to provision a new hire's equivalent, or to know
--     what to collect when someone leaves. (Example: Maria Lopez.)
...
--     Catches the new hire who started but has no equipment yet.
...
--     These need to be recovered and wiped. Should ideally return nothing.
